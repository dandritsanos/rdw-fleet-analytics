...
32 columns as delivered. Discovery on 2026-07-28 (50-row sample) found only 10; full schema confirmed via `describe raw.fuel` against the 300K-row extract on 2026-07-31. Sample-based discovery under-covers this table specifically — RDW populates different fuel-attribute columns depending on powertrain, and the sample lacked coverage across all fuel types. `describe` on the landed table is the source of truth going forward, not the discovery script.
...
| `co2_uitstoot_gecombineerd` | `co2_combined_gkm` | integer | |
...
Columns present but unused (WLTP variants, noise-level fields): `emissie_co2_gecombineerd_wltp`, `brandstof_verbruik_gecombineerd_wltp`, `geluidsniveau_stationair`, `geluidsniveau_rijdend`, `toerental_geluidsniveau`, others. Out of scope for the current analysis and not selected in staging.
